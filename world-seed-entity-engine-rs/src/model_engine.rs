...
use minestom::{jni_utils::{get_env, JavaObject}, material::Material, Result};
use std::path::Path;
...
impl ModelEngine {
    pub fn set_model_material(model_material: Material) -> Result<()> {
        ...
        let material_obj = env
            .call_static_method(
                ...
                &[JValue::from(&env.new_string(model_material.to_java_name())?)],
            )?
            .l()?;
    ...
    pub fn load_mappings<A, B>(mappings_path: A, models_path: B) 
    -> Result<()> where A: AsRef<Path>, B: AsRef<Path> {
        let mut env = get_env()?;

        // Create a FileInputStream for the mappings file
        let mapping_str = mappings_path.as_ref().to_str()
        ...
        let models_str = models_path.as_ref().to_str()
            .expect("Invalid models path");
        let j_models_str = env.new_string(models_str)?;
                // Create an empty String[] for varargs (no init element)
        let empty_str_array = env.new_object_array(
            0,
            "java/lang/String",
            JObject::null(),
        )?;
